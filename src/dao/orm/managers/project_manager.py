...
class ProjectManager(BaseManager):
    ...
    async def create_user_default_todo_project(self, user_id: int, project_name="默认待办", project_desc="默认待办"):
        ...
        await UserProjectManager().bind_user_project(user_id, project_id)
        return project_id

    async def create_user_project(self, user_id: int, project_info: dict):
        project_id = await self.add(project_info)
        await UserProjectManager().bind_user_project(user_id, project_id)
        return project_id

    async def delete_projects(self, project_ids: list[int]):
        del_ret = await self.bulk_delete_by_ids(project_ids)
        await UserProjectManager().delete_by_project_ids(project_ids)
        return del_ret
